...
A formal proof is given in the Appendix. The idea can be illustrated very simply by reference to the vowel system in Fig.~\ref{fig:lattice}. Let $\mathbf{e}_1 =$ \phonfeat{+front} and $\mathbf{e}_1 =$ \phonfeat{+round}. Then \begin{itemize}
...
The reasoning is intimately related to that of Part 2. Suppose that $A$ is a daughter of three distinct classes $B$, $C$, and $D$. Then $A = B \cap C$, but also $A = B \cap D$, which implies that $C = D$, contra hypothesis. By similar reasoning (induction), $A$ cannot be the daughter of more than 3 classes. % what's up with the null set? something wrong with the logic here
