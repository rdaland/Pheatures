...
\vspace{\baselineskip} \noindent \begin{algorithmic}
    ...
    \STATE
    \STATE $\mathcal Q \leftarrow \mathcal C_\cap$
    \STATE $\textsc{SortLargeToSmall}(\mathcal{Q})$
    \STATE $\mathcal F \leftarrow \varnothing$
    \STATE
    \WHILE{$\mathcal Q \neq \varnothing$}
        \STATE $X \leftarrow \textsc{dequeue}(\mathcal Q)$
...
%TODO: Put in proof that BFS works here. Sketch:
% Assume class Z adds complement wrt to parent Y, Z', that is parent of X. Thus X must be descedent of Y (either daughter or further).

%if X is not daughter of Y, then Z' will be added before X is processed, so no problem.

%If X is daughter of Y, then what?
%- if X union Z is not empty, then Z' cannot be a parent of X.
%- if X was already processed, then X' would have been added, and because X union Z is empty, X' would be Z's parent, meaning Z is not the child of Y. Contradiction.

% Adding nodes in BFS ordered from largest to smallest results in fewer features, can demonstrate. Why?
% This is maybe a testable empirical question?
