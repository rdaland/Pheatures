...
\date{\vspace{-10ex}}							% Activate to display a given date or no date
...
\noindent Let $\mathbf{d}_1 =$ \featmat{+front} and $\mathbf{d}_2 =$ \featmat{+round}. Then we have: \begin{itemize}
    \item $\langle \mathbf{d}_1 \rangle = \langle$\featmat{+front}$\rangle =$ \{\textipa{\o}, \textipa{y}, \textipa{e}, \textipa{i}\}
    \item $\langle \mathbf{d}_2 \rangle = \langle$\featmat{+round}$\rangle =$ \{\textipa{\o}, \textipa{o}, \textipa{y}, \textipa{u}\}
    \end{itemize}
...
\begin{itemize}
    \item $\langle \mathbf{d}_1 \rangle \cap \langle \mathbf{d}_2 \rangle =  \langle$\featmat{+front}$\rangle \cap \langle$\featmat{+round}$\rangle =$ \{\textipa{\o}, \textipa{y}, \textipa{e}, \textipa{i}\} $\cap$ \{\textipa{\o}, \textipa{o}, \textipa{y}, \textipa{u}\} = \{\textipa{\o}, \textipa{y}\}
    \item $\langle \mathbf{d}_1 \cup \mathbf{d}_2 \rangle = \langle$\featmattwo{+front}{+round}$\rangle =$ \{\textipa{\o}, \textipa{y}\}
\end{itemize}
...
We have not discussed the possibility of applying `$-$' feature values to complements with respect to an ancestor other than the parent or $\Sigma$. This bears directly on where underspecification should occur. For example, we may want to specify every coronal obstruent as either \featmat{+strident} or \featmat{-strident}, and all non-coronals as \featmat{0strident}. It is less clear, though, how we should specify coronal sonorants for \featmat{strident}. We do not put forth a concrete proposal for whether and how this should be done, but leave it as a possible area for future research informed by empirical phonological evidence.

Finally, it is worth touching briefly upon the challenges for underspecification theory posed by Richness of the Base \cite{PrinceSmolensky1993}. This stipulates that there are no constraints on the input, and so a grammar must be able to deal sensibly with both fully specified and underspecified forms. Certain combinations of features generated by these algorithms are necessary and sufficient for a featural specification to surface as a particular segment, regardless of what else is specified (e.g., \featmat{+low} for /a/ in the vowel inventory). Faithfulness constraints on these contrastive features and markedness constraints against spurious feature combinations would be sufficient to eliminate undesirable candidates \cite{Archangeli2011}. Further, if phonotactic constraints are learned from positive input data \cite<e.g.,>{HayesWilson2008}, underspecified features serve an important role in constraining the generalizations the phonotactic learner may make. Finally, we note that language-specific features complicate the handling of non-native input forms. We follow \citeA{Hall2007} in suggesting that the answer for this lies in a better understanding of how speakers map acoustic input onto the phonological representations of their language.
