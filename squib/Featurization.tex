...
\begin{abstract}
Given a set of phonological features, we can enumerate a set of phonological classes. Here we consider the inverse of this problem: given a set of phonological classes, can we work backwards to a feature system? We show that this is indeed possible, using a collection of algorithms that assign features to a set of input classes and differ in terms of what types of features are permissible. This work bears on theories of both emergent and universal features, serves as a useful component in computational models of feature learning, and provides testable predictions on the featurizations available to learners.
\end{abstract}
...
%	Fig. 4 (done) -- Yield of the privative specification algorithm (to vowel inventory)
% 	Fig. 5 (done) -- Yield of the contrastive underspecification algorithm (to vowel inventory)
%	Fig. 6 (TODO) -- Class system and featurization yielded by contrastive specification. Need to colorize.
%	Fig. 7 (done) -- Class system and featurization yielded by full specification.
%	Fig. 8 (done) -- Big alphabet (contrastive underspecification with privative, binary, and ternary features)
%	Fig. 9 (done) -- Simple class system to illustrate BFS
%	Fig. 10 (done) -- Featurization of simple class system without BFS
%	Fig. 11 (done) -- Featurization of simple class system with BFS
%	Fig. 12 (done) -- Simple class system to illustrate sibling ordering considerations
%	Fig. 13 (done) -- Featurization of simple class system if siblings are processed in arbitrary order
%	Fig. 14 (done) -- Featurization of simple class system if siblings are processed simultaneously
%	Fig. 15 (done) -- Topological plot of the full specification vowel inventory output.
...
\end{figure}
% ROBERT SEZ: 
%	I changed Fig. 1 to have rectangular box for every class.
%    Note that on my machine I have not been able to figure out how to get dot to map directly to PNG.
%    The [dpi = ...] argument produces invalid SVG. So for now I will leave that part out.
...
We illustrate this algorithm using the same vowel system used in the previous section. The featurization using this algorithm is shown in Figure~\ref{fig:vowel_inventory_contrastive_under}. Note that now only nine features are required. The segment /a/, which was \phonfeat{+low} under the privative algorithm, has instead been featurized as \phonfeat{--nonlow} here. This is because the low and non-low classes are complements with respect to their parent ($\Sigma$), and both are present in the input. Contrastive \phonfeat{nonlow} is doing the work of privative \phonfeat{low} and privative \phonfeat{nonlow} together, so there is no need for a contrastive \phonfeat{low} feature.

An additional point of note is that the \{y\} class gets \phonfeat{-\textipa{I}}, \phonfeat{-\textipa{\oe}}, and \phonfeat{-u} values, and these features are now ternary. This occurs because /y/ is a complement to the single parent that motivates the addition of these features. Those happen to be the classes that are `added' by featural closure here (although that is not always the case). Note that the remaining features (\phonfeat{front}, \phonfeat{round}, \phonfeat{high}, \phonfeat{\textipa{E}}, \phonfeat{o}) are still privative, because their respective complements are not present in the input.
...
% FIGURE 6
% TODO: We need to colorize the boxes to indicate new classes, and colorize the features too! Also increase DPI
\begin{figure}[htb!]
	\centering
...
This featurization is illustrated by Figure~\ref{fig:vowel_inventory_full}, which uses the same vowel system from previous sections. There is an important difference in the way this plot is organized. We may consider two types of plots when plotting featural relations: a \emph{topological} plot, which plots relationships between classes using the familiar notions of parent/child relationship developed throughout this paper, and a \emph{featural} plot, where classes corresponding [+F] and [-F] feature/value pairs are plotted as siblings at the same rank in the hierarchy. In all the cases considered so far, these two plotting strategies have resulted in identical outcomes. With the full specification algorithm on the simple vowel inventory, this is no longer the case.

The plot in Figure~\ref{fig:vowel_inventory_full} shows the featural plot. The topological plot is shown in Appendix~\ref{app:topological}. The most salient difference between the two is that /a/ is plotted as a child of $\Sigma$ in the featural plot. This reflects the point at which it is necessary to create a \phonfeat{nonlow} feature to distinguish the \phonfeat{+nonlow} segments from the full set of segments. The topological plot shows that /a/ is not a in fact child of $\Sigma$, and actually has multiple parents. However, /a/ is identified as \phonfeat{-nonlow} because it is the complement of the class of \phonfeat{nonlow} vowels, and we do not consider the number of parents when assigning $-$ values to a complement.
...
\begin{figure}[htb!]
	\centering
	\includegraphics[width=\textwidth]{vowel_inventory_full_FEATURAL.png}
...
In any case, what must be learned under models of feature learning, even those that do not incorporate distributional learning, are \textit{classes}, with features subsequently derived from their relation. We see the question of how phonological classes are learned as one of great interest, and we hope that the work here serves as a useful component in learning models and tool in future empirical investigations.

\FloatBarrier
\section{Conclusion}
\label{sec:conclusion}

This paper provides a detailed formalization of the properties of phonological feature systems and describes algorithms for efficiently calculating various types of featurizations of a set of input classes. An implementation of these algorithms is available for use in further research. We believe that this work provides a stronger formal grounding for the study of phonological features, may serve as a useful component in computational models of feature learning, and that the predictions made by the algorithms for various inputs and featurization types provide useful, testable empirical hypotheses for future experimental phonological research.
...
\subsection{A topological version of the full specification output plot}
\label{app:topological}

\begin{figure}[htb!]
\centering
\includegraphics[width=\textwidth]{vowel_inventory_full_TOPOLOGICAL.png}
\caption{The topological graph of the output of the full specification algorithm on the vowel inventory. Here the parent/child relationship is maintained in the graph, but featural siblings (i.e. +/- pairs) are not necessarily plotted at the same level.}
