...
TODO: [1] you can build a low-expressive privative system with $n$ features, where $n = | \Sigma |$ \begin{itemize}
    \item just make each feature be the form $f_i = \{\sigma_i\}$ where $\sigma_i$ ranges over the segments of $\Sigma$
    \end{itemize}
    
TODO: [2] you can also build a fully-expressive privative system with $n$ features \begin{itemize}
    \item just make each feature have the form $f_i = \Sigma \setminus \{\sigma_i\}$ where $\sigma_i$ ranges as before $\Sigma$
    \item in that case to get any class you just figure out what isn't there and intersect all of the corresponding missing-singleton sets
    \item for full specification, the theoretical minimum is ceiling(log 2 n) -- each top-level feature partitions the space in half, and the partitions are not just independent but orthogonal
    \end{itemize}
    
TODO: [3] in general, you can mimic or achieve the functionality of more expressive algorithms by including the relevant complement classes in the input \begin{itemize}
    \item for example, privative specification yields the same class system as full specification, if you include every relevant complement class
    \item contrastive underspecification, contrastive specification, and full specification yield the same featurization if you include every relevant complement class
    \item but only contrastive underspecification allows `$+$' and `$-$' values without forcing some kind of complement class
    \item thus, contrastive underspecification gives the user the most control in how expressive the system is -- the more complement classes you include, the closer you get to the maximally expressive full specification system
    \end{itemize}

\subsection{Relation to feature theory}

TODO: [4] as shown above (and I think proven by other computationalists), these feature systems are all able to achieve equal expressivity (in the sense of, what classes do they generate) \begin{itemize}
    \item though of course, the amount of expressiveness each achieves is different depending on the input
    \end{itemize}
What else matters for the formal structure of feature systems? The principle thing seems to be, the extent to which the learned features render the phonological grammar simpler or more insightful (e.g. sameness of plus/minus values for harmony). Another consideration might be learnability, the topic of the next subsection
